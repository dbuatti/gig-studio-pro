...
const SongStudioModal: React.FC<SongStudioModalProps> = ({ 
  ...
  useEffect(() => {
    if (song && isOpen) {
      // RESET MODAL STATE FOR NEW SONG
      setActiveTab('audio');
  ...
  const handleProSync = async () => {
    ...
    showSuccess("Pro Sync triggered for current song.");
  };
        ...
        <div className="flex h-[90vh] min-h-[800px] overflow-hidden">
          ...
          <div className="flex-1 flex flex-col min-w-0">
            ...
            <div className="flex-1 overflow-y-auto relative">
                ...
                <div className="p-12 space-y-12 animate-in fade-in duration-500">
                  <div className="flex items-center justify-between">
                    ...
                    <div className="flex gap-4">
                      ...
                      <Button 
                        variant="outline"
                        onClick={() => window.open(`https://www.youtube.com/results?search_query=${encodeURIComponent((formData.artist || '') + ' ' + (formData.name || '') + ' studio version audio')}`, '_blank')}
              ...
              {activeTab === 'visual' && videoId && (
                <div className="p-12 h-full animate-in fade-in duration-500">
                   <div className="aspect-video w-full rounded-[3rem] overflow-hidden shadow-2xl border border-white/10 bg-black">
                    <iframe 
                      width="100%" height="100%" 
                      src={`https://www.youtube.com/embed/${videoId}?autoplay=1&mute=1`}
                      title="Reference Video" frameBorder="0" allowFullScreen
                    />
                  </div>
                </div>
              )}
              ...
              {activeTab === 'library' && (
                <div className="p-12 grid grid-cols-2 gap-8 animate-in fade-in duration-500">
                  <div className="p-8 rounded-[2.5rem] border bg-white/5 border-white/10 shadow-xl flex flex-col justify-between h-72">
                    <div className="bg-emerald-600 p-4 rounded-2xl shadow-lg w-fit">
                      <FileText className="w-8 h-8" />
                    </div>
                    <div className="space-y-2">
                      <Label className="text-[10px] font-black uppercase tracking-[0.2em] text-emerald-400">Stage Chart / PDF</Label>
                      <p className="text-2xl font-black">{formData.pdfUrl ? "Performance_Chart" : "Not Linked"}</p>
                      {formData.pdfUrl && <Button variant="ghost" size="sm" onClick={() => setPreviewPdfUrl(formData.pdfUrl!)} className="text-emerald-400 p-0">Preview Full Stage View</Button>}
                    </div>
                  </div>

...
import { 
  ...
  ClipboardPaste, AlignLeft, Apple, Hash, Music2, Share2
} from 'lucide-react';
        ...
        <div className="flex h-[90vh] min-h-[800px]">
          <div className="w-96 bg-slate-900/50 border-r border-white/5 flex flex-col">
            <div className="p-8 border-b border-white/5 bg-black/20">
              <div className="flex items-center justify-between mb-4">
                ...
                <Button onClick={handleProSync} className="bg-indigo-600 hover:bg-indigo-700 h-8 px-3 text-[9px] font-black uppercase tracking-widest gap-2 shadow-lg shadow-indigo-600/20">
            ...
            <div className="flex-1 overflow-y-auto p-8 space-y-10">
                    ...
                    <div className="flex gap-2">
                      <Tooltip>
                        <TooltipTrigger asChild>
                          <button onClick={() => handleAutoSave({ key_preference: currentKeyPreference === 'sharps' ? 'flats' : 'sharps' })} className={cn("p-1.5 rounded-lg border transition-all flex items-center gap-2 px-3", formData.key_preference ? "bg-indigo-600 border-indigo-500 text-white shadow-lg" : "bg-white/5 border-white/10 text-slate-500")}>
                      ...
                      <Tooltip>
                        <TooltipTrigger asChild>
                          <button onClick={() => updateHarmonics({ isKeyLinked: !formData.isKeyLinked })} className={cn("p-1.5 rounded-lg border transition-all", formData.isKeyLinked ? "bg-indigo-600 border-indigo-500 text-white shadow-lg shadow-indigo-600/20" : "bg-white/5 border-white/10 text-slate-500")}>
                            <LinkIcon className="w-3.5 h-3.5" />
                          </button>
                        </TooltipTrigger>
                        <TooltipContent className="text-[10px] font-black uppercase">{formData.isKeyLinked ? "Keys Linked to Pitch" : "Pitch is Independent"}</TooltipContent>
                      </Tooltip>
                    </div>
                  </TooltipProvider>
                </div>
              ...
              <div className="space-y-4">
                ...
                <div className="grid grid-cols-1 gap-2.5">
                  {RESOURCE_TYPES.map(res => {
                    const isActive = formData.resources?.includes(res.id) || (res.id === 'UG' && formData.ugUrl) || (res.id === 'LYRICS' && formData.lyrics) || (res.id === 'PDF' && formData.pdfUrl) || (res.id === 'LEAD' && formData.leadsheetUrl);
                    return (
              ...
              <div className="space-y-4">
                ...
                <div className="flex flex-wrap gap-2 mb-3">{(formData.user_tags || []).map(t => (
          ...
          <div className="flex-1 flex flex-col">
            <div className="h-20 border-b border-white/5 flex items-center px-12 justify-between bg-black/20 shrink-0">
              <div className="flex gap-12">{['audio', 'details', 'lyrics', 'visual', 'library'].map((tab) => (
                <button key={tab} onClick={() => setActiveTab(tab as any)} className={cn("text-xs font-black uppercase tracking-[0.4em] h-20 transition-all border-b-4", activeTab === tab ? "text-indigo-400 border-indigo-500" : "text-slate-500 border-transparent hover:text-white")}>{tab.toUpperCase()} ENGINE</button>
              ))}</div>
            ...
            <div className="flex-1 overflow-y-auto p-12">
              {activeTab === 'audio' && (
                <div className="space-y-12 animate-in fade-in duration-500">
                  <div className="flex items-center justify-between">
                    ...
                    <Button variant="outline" onClick={handleYoutubeSearch} className="bg-red-600/10 border-red-600/20 text-red-600 hover:bg-red-600 hover:text-white font-black uppercase tracking-widest text-[9px] h-10 gap-2 px-6 rounded-xl transition-all"><Youtube className="w-3.5 h-3.5" /> Discovery Mode</Button>
                  </div>
                  <div className="bg-slate-900/50 rounded-[3rem] border border-white/5 p-12 space-y-12">
                    <div className="h-40"><AudioVisualizer analyzer={analyzerRef.current} isActive={isPlaying} /></div>
                    {formData.previewUrl ? (
                      <>
                        <div className="space-y-8">
                          ...
                          <Slider value={[progress]} max={100} step={0.1} onValueChange={(v) => { const p = v[0]; setProgress(p); const offset = (p / 100) * duration; playbackOffsetRef.current = offset; if (isPlaying && playerRef.current) { playerRef.current.stop(); playbackStartTimeRef.current = Tone.now(); playerRef.current.start(0, offset); } }} />
                        </div>
                        <div className="flex items-center justify-center gap-12">
                           <Button variant="ghost" size="icon" onClick={stopPlayback} className="h-20 w-20 rounded-full border border-white/5 hover:bg-white/5 hover:scale-110 transition-all"><RotateCcw className="w-8 h-8" /></Button>
                      ...
                      <div className="flex flex-col items-center justify-center py-12 space-y-6">
                        ...
                        <div className="text-center space-y-2">
                           ...
                           <p className="text-sm text-slate-500 max-w-sm">No performance track is linked.</p>
                        </div>
                        <Button onClick={handleYoutubeSearch} className="bg-red-600 hover:bg-red-700 font-black uppercase tracking-widest text-xs h-12 gap-3 px-8 rounded-2xl shadow-xl shadow-red-600/20"><Search className="w-4 h-4" /> Discover on YouTube</Button>
                      </div>
                    )}
                  </div>
                </div>
                ...
                <div className="space-y-12 animate-in fade-in duration-500">
                  <div className="flex items-center justify-between">
                    <div>
                      ...
                      <p className="text-sm text-slate-500 mt-2">Centralized management for all song assets.</p>
                    </div>
                    <Button onClick={handleDownloadAll} className="bg-indigo-600 hover:bg-indigo-700 font-black uppercase tracking-widest text-xs h-12 gap-3 px-8 rounded-2xl shadow-xl shadow-indigo-600/20"><Download className="w-4 h-4" /> Download All</Button>
                  </div>
                  <div className="grid grid-cols-2 gap-8">
                    <div className={cn("group p-8 rounded-[2.5rem] border transition-all relative flex flex-col justify-between h-72", formData.ugUrl || (formData.artist && formData.name) ? "bg-white/5 border-white/10 shadow-xl" : "bg-white/5 border-white/5 opacity-40 border-dashed")}>
                      <div className="flex items-center justify-between">
                        <div className="bg-orange-600 p-4 rounded-2xl shadow-lg shadow-orange-600/20"><Link2 className="w-8 h-8" /></div>
                        <div className="flex gap-2">
                          <TooltipProvider>
                            <Tooltip>
                              <TooltipTrigger asChild>
                                <Button variant="ghost" size="icon" className="h-10 w-10 text-indigo-400 hover:bg-indigo-600 hover:text-white transition-all rounded-xl border border-indigo-500/20" onClick={handleOnSongExport}>
                                  <Share2 className="w-4 h-4" />
                                </Button>
                              </TooltipTrigger>
                              <TooltipContent className="text-[10px] font-black uppercase">Import into OnSong App</TooltipContent>
                            </Tooltip>
                            <Button variant="ghost" size="icon" className="h-10 w-10 text-orange-400 hover:bg-orange-600 hover:text-white transition-all rounded-xl border border-orange-500/20" onClick={handleUgPrint}><Printer className="w-4 h-4" /></Button>
                            <Button variant="ghost" size="icon" className="h-10 w-10 text-orange-400 hover:bg-orange-600 hover:text-white transition-all rounded-xl border border-orange-500/20" onClick={handlePasteUgUrl}><ClipboardPaste className="w-4 h-4" /></Button>
                            <Button variant="ghost" size="icon" className="h-10 w-10 text-orange-400 hover:bg-orange-600 hover:text-white transition-all rounded-xl" onClick={handleUgAction}>{formData.ugUrl ? <LinkIcon className="w-4 h-4" /> : <ExternalLink className="w-4 h-4" />}</Button>
                          </TooltipProvider>
                        </div>
                      </div>
                    ...
                    <div className={cn("group p-8 rounded-[2.5rem] border transition-all relative flex flex-col justify-between h-72", formData.pdfUrl ? "bg-white/5 border-white/10 shadow-xl" : "bg-white/5 border-white/5 opacity-40 border-dashed")}>
                      <div className="flex items-center justify-between"><div className="bg-emerald-600 p-4 rounded-2xl shadow-lg shadow-emerald-600/20"><FileText className="w-8 h-8" /></div></div>
                      <div className="space-y-2 mt-6"><Label className="text-[10px] font-black uppercase tracking-[0.2em] text-emerald-400">Stage Chart / PDF</Label><p className="text-2xl font-black tracking-tight">{formData.pdfUrl ? "Performance_Chart" : "Not Linked"}</p></div>
                    </div>
                  </div>
                </div>
              )}
              ...
              {activeTab === 'lyrics' && (
                <div className="space-y-10 animate-in fade-in duration-500 h-full flex flex-col">
                  <div className="flex items-center justify-between">
                    <div><h3 className="text-lg font-black uppercase tracking-[0.3em] text-pink-400">Lyrics Engine</h3></div>
                    <div className="flex gap-3">
                      <Button variant="outline" onClick={handleMagicFormatLyrics} className="bg-indigo-600/10 border-indigo-600/20 text-indigo-600 hover:bg-indigo-600 hover:text-white font-black uppercase tracking-widest text-[9px] h-10 gap-2 px-6 rounded-xl transition-all"><Sparkles className="w-3.5 h-3.5" /> Magic Format</Button>
                    </div>
                  </div>
                  <Textarea placeholder="Paste lyrics here..." value={formData.lyrics || ""} onChange={(e) => handleAutoSave({ lyrics: e.target.value })} className="flex-1 min-h-[400px] bg-white/5 border-white/10 text-xl leading-relaxed rounded-[2.5rem] p-10 font-medium whitespace-pre-wrap" />
                </div>
              )}
            </div>
          </div>

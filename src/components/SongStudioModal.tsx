                 ...
                 <div className="text-center space-y-2 max-w-sm px-6">
                   <p className="text-sm font-black uppercase tracking-[0.2em] text-white">
                     {isUploading ? 'Linking Master...' : 
                      isSyncingAudio ? (syncStatus || 'Extracting Audio...') :
              ...
              {activeTab === 'audio' && (
                <div className={cn("space-y-6 md:space-y-12 animate-in fade-in duration-500")}>
                  <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                    ...
                    <Button variant="outline" onClick={handleYoutubeSearch} disabled={isSearchingYoutube} className="bg-red-600/10 border-red-600/20 text-red-600 hover:bg-red-600 hover:text-white font-black uppercase text-[9px] h-10 gap-2 px-4 rounded-xl">
                      {isSearchingYoutube ? <Loader2 className="w-3.5 h-3.5 animate-spin" /> : <Youtube className="w-3.5 h-3.5" />} Discovery
                    </Button>
                  </div>
              ...
              {activeTab === 'visual' && (
                <div className="space-y-10 animate-in fade-in duration-500 h-full flex flex-col">
                  ...
                  <div className="flex flex-col md:flex-row gap-4 shrink-0">
                     <Input placeholder="Search song or paste URL..." value={formData.youtubeUrl} onChange={(e) => handleAutoSave({ youtubeUrl: e.target.value })} onKeyDown={(e) => e.key === 'Enter' && performYoutubeDiscovery(formData.youtubeUrl || '')} className="flex-1 bg-slate-900 border-white/10 h-14 px-6 rounded-xl" />
                     <div className="flex gap-3">
                        <Button onClick={() => handleSyncYoutubeAudio()} disabled={isSyncingAudio || !formData.youtubeUrl} className="bg-indigo-600 hover:bg-indigo-700 h-14 px-8 rounded-xl font-black uppercase tracking-widest text-[10px] gap-3">
                          {isSyncingAudio ? <Loader2 className="w-4 h-4 animate-spin" /> : <Zap className="w-4 h-4" />} EXTRACT
                        </Button>
                        <Button variant="outline" onClick={() => performYoutubeDiscovery(formData.youtubeUrl || `${formData.artist} ${formData.name} official video`)} disabled={isSearchingYoutube} className="bg-red-950/30 border-red-900/50 text-red-500 h-14 px-8 rounded-xl font-black uppercase tracking-widest text-[10px] gap-3">
                          {isSearchingYoutube ? <Loader2 className="w-3.5 h-3.5 animate-spin" /> : <Search className="w-4 h-4" />} SEARCH
                        </Button>
                     </div>
                  </div>
                  {ytResults.length > 0 && <YoutubeResultsShelf results={ytResults} currentVideoId={currentVideoId} onSelect={handleSelectYoutubeVideo} onSyncAndExtract={(url) => handleSyncYoutubeAudio(url)} isLoading={isSearchingYoutube} isExtracting={isSyncingAudio} />}

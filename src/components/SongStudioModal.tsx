...
const SongStudioModal: React.FC<SongStudioModalProps> = ({ 
  ...
  const handleSelectProSync = async (itunesData: any) => {
    ...
    try {
      ...
      const { data, error } = await supabase.functions.invoke('enrich-metadata', {
        body: {
          queries: [`${itunesData.trackName} by ${itunesData.artistName}`]
        }
      });
  ...
  useEffect(() => {
    if (song && isOpen) {
      const initialData: Partial<SetlistSong> = {
        name: song.name || "",
        artist: song.artist || "",
        bpm: song.bpm || "",
          ...
          <div className="absolute inset-0 z-[100] bg-black/90 backdrop-blur-xl animate-in fade-in zoom-in duration-300 flex flex-col p-6 md:p-12">
            <div className="flex items-center justify-between mb-8">
              ...
              <Button 
                ...
                className="h-10 w-10 bg-white/5 rounded-full hover:bg-white/10"
              >
            ...
            <>
              ...
              <div className="text-center space-y-2 max-w-sm px-6">
                <p className="text-sm font-black uppercase tracking-[0.2em] text-white">
                  {isUploading ? 'Linking Master...' : 'Analyzing Library...'}
                </p>
              </div>
            </>
          </div>
              ...
              <div className="p-8 border-b border-white/5 bg-black/20 shrink-0 relative">
                ...
                <div className="mt-4 flex items-center gap-2">
                  <div className="flex-1 h-1 bg-white/5 rounded-full overflow-hidden">
                    <div 
                      className={cn("h-full transition-all duration-700", readinessColor)} 
                      style={{ width: `${readiness}%` }} 
                    />
                  </div>
                </div>
              ...
              {!isMobile && (
                <div className="flex items-center gap-6">
                  <Button 
                    ...
                    size="sm" 
                    onClick={onClose}
                    className="text-slate-400 hover:text-white font-black uppercase tracking-[0.3em] text-xs"
                  >
                    Close
                  </Button>
                </div>

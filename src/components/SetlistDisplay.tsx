...
import {
  ...
  Star, Filter, AlertTriangle, Loader2, CloudDownload, Edit, ListMusic, ArrowRight, Trash2, Wand2, X, CheckCircle, GripVertical
} from 'lucide-react';
...
const SetlistDisplay: React.FC<SetlistDisplayProps> = ({
  ...
  const filteredSongs = useMemo(() => {
    ...
    if (activeFilters.minReadiness !== undefined) {
      filtered = filtered.filter(s => calculateReadiness(s) >= activeFilters.minReadiness!);
    }
    if (activeFilters.maxReadiness !== undefined) {
            ...
            {filteredSongs.length === 0 ? (
              <TableRow>
                <TableCell colSpan={5} className="py-20 text-center opacity-30">
                  ...
                  <p className="text-sm font-black uppercase tracking-widest text-muted-foreground">No Songs Match Criteria.</p>
                </TableCell>
              </TableRow>
            ) : (
                  ...
                  >
                      ...
                      <div className="flex flex-col gap-1">
                        <div className="flex items-center gap-3">
                          <h4 className="text-base font-black tracking-tight leading-none flex items-center gap-2">
                            {song.name}
                            {readiness === 100 && <Check className="w-4 h-4 text-emerald-500 fill-emerald-500/20" />}
                    ...
                    <TableCell className="px-6 text-right pr-10">
                      <div className="flex items-center justify-end gap-2 h-full">
                        ...
                        />
                        <Button variant="ghost" size="icon" className="h-9 w-9 rounded-xl text-muted-foreground hover:text-indigo-600 hover:bg-indigo-50 transition-colors" onClick={(e) => { e.stopPropagation(); onEdit(song); }}>

...
interface SongSearchProps {
  ...
  onAddToSetlist: (url: string, name: string, artist: string, youtubeUrl?: string, ugUrl?: string, appleMusicUrl?: string, genre?: string) => void;
...
const SongSearch: React.FC<SongSearchProps> = ({ onSelectSong, onAddToSetlist, externalQuery }) => {
  ...
  return (
    <div className="space-y-4">
      <form onSubmit={handleSearch} className="flex gap-2">
        <div className="relative flex-1">
          ...
          <Input 
            ...
            className="pl-9 h-11 border-border bg-background dark:bg-slate-900 focus-visible:ring-indigo-500"
            value={query}
      ...
      {results.length > 0 && (
        <Card className="border-border dark:border-slate-800 overflow-hidden shadow-inner bg-background dark:bg-slate-900/30">
          <ScrollArea className="h-[450px]">
            <div className="p-2 space-y-1">
              <TooltipProvider>
                {results.map((song) => (
                  <div key={song.trackId} className="flex flex-col border-b last:border-0 border-border dark:border-slate-800">
                    <div className="w-full flex items-center gap-3 p-2 hover:bg-accent dark:hover:bg-slate-800 rounded-lg transition-all group">
                      <button
                      ...
                      >
                        ...
                        <div className="flex-1 min-w-0">
                          <p className="text-sm font-bold truncate text-foreground dark:text-white">{song.trackName}</p>
                          ...
                          <TooltipTrigger asChild>
                            <button 
                              ...
                              className={cn(
                                ...
                                expandingId === song.trackId ? "bg-indigo-600 text-white shadow-lg" : "hover:bg-accent dark:hover:bg-slate-800 text-muted-foreground"
                              )}
                            >
                    ...
                    {expandingId === song.trackId && (
                      <div className="px-3 pb-3 animate-in slide-in-from-top-2 duration-300">
                        <div className="p-4 bg-background dark:bg-slate-900 rounded-xl border-2 border-indigo-200 dark:border-indigo-900/20 shadow-sm space-y-4">
                          <div className="flex items-center justify-between border-b pb-2 border-border dark:border-slate-800">
                              ...
                              <div className="flex gap-2">
                                <div className="relative flex-1">
                                  ...
                                  <Input 
                                    ...
                                    className="h-9 pl-7 text-[10px] bg-background dark:bg-slate-950 border-border dark:border-slate-800 text-foreground dark:text-white"
                                    value={manualYtUrl}
                                ...
                                <Button 
                                  variant="outline" 
                                  size="sm" 
                                  className="h-9 border-border dark:border-slate-800 text-muted-foreground hover:bg-red-50 hover:text-red-600 text-[10px] font-bold uppercase"
                            ...
                            <div className="space-y-2">
                              ...
                              <div className="flex gap-2">
                                ...
                                <Button 
                                  variant="outline" 
                                  size="sm" 
                                  className="h-9 border-border dark:border-slate-800 text-muted-foreground hover:bg-orange-50 hover:text-orange-600 text-[10px] font-bold uppercase"

...
const AdminPanel: React.FC<AdminPanelProps> = ({ isOpen, onClose }) => {
  ...
  const handleRefreshCookies = async () => {
    if (!cookieText.trim()) {
      showError("Please paste the Netscape cookie file content.");
      return;
    }
    ...
    try {
      ...
      if (error) {
        // Supabase function errors can be complex, try to parse the body
        let errorData = error;
        try {
          // If it's a Response object or has a context we can read
          ...
          <div className="p-8 space-y-6">
            ...
            {lastError && (
              <div className="bg-red-500/10 border border-red-500/20 rounded-2xl p-6 space-y-3 animate-in fade-in slide-in-from-top-2">
                ...
                <p className="text-[10px] text-slate-500 italic">
                  If you see "Repository rule violations", look for a "bypass_placeholder_url" in the trace above.
                </p>
              </div>
            )}
            ...
            <div className="flex items-center gap-4 p-5 bg-indigo-500/10 rounded-2xl border border-indigo-500/20">
              ...
              <p className="text-[11px] text-indigo-300 font-medium leading-relaxed">
                Syncing triggers an automatic container rebuild on Render. The audio engine will be unavailable during the "Deploying" state.
              </p>
            </div>
          </div>
        </ScrollArea>

...
const GigSessionManager: React.FC<GigSessionManagerProps> = ({ setlistId }) => {
  ...
  return (
    <div className="space-y-6 bg-secondary p-6 rounded-[2rem] border border-border">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h4 className="text-sm font-black uppercase tracking-tight text-foreground">Gig Access Codes</h4>
            <p className="text-[10px] text-muted-foreground font-bold uppercase tracking-widest">Public Setlist Portals</p>
          </div>
        </div>
      </div>
      ...
      <div className="space-y-2">
        {loading ? (
          <div className="py-4 text-center"><Loader2 className="w-6 h-6 animate-spin text-muted-foreground mx-auto" /></div>
        ) : sessions.length === 0 ? (
          <div className="py-8 text-center border-2 border-dashed border-border rounded-2xl">
            <Calendar className="w-8 h-8 text-muted-foreground mx-auto mb-2" />
            <p className="text-[10px] font-black uppercase text-muted-foreground tracking-widest">No active sessions</p>
          </div>
        ) : (
          sessions.map(session => (
            <div key={session.id} className="p-4 bg-card border border-border rounded-2xl flex items-center justify-between group">
              <div className="flex items-center gap-4">
                <button 
                  onClick={() => toggleSession(session.id, session.is_active)}
                  className={cn(
                    "w-3 h-3 rounded-full transition-all",
                    session.is_active ? "bg-emerald-500 shadow-[0_0_8px_rgba(16,185,129,0.5)]" : "bg-muted-foreground"
                  )}
                />
                <div>
                  <p className="text-sm font-black uppercase tracking-widest text-foreground">{session.access_code}</p>
                  <p className="text-[9px] font-bold text-muted-foreground uppercase">{session.is_active ? 'Active' : 'Offline'}</p>
                </div>
              </div>
              ...
              <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                ...
                >
                  {copiedId === session.id ? <Check className="w-3.5 h-3.5 text-emerald-500" /> : <Copy className="w-3.5 h-3.5 text-muted-foreground" />}
                </Button>
                <Button 
                  variant="ghost" size="icon" className="h-8 w-8 rounded-lg"
                ...
                <Button 
                  variant="ghost" size="icon" className="h-8 w-8 rounded-lg text-destructive hover:bg-destructive/10"

...
export interface SetlistSong {
  ...
  preferred_view?: 'visualizer' | 'lyrics' | 'pdf' | 'leadsheet' | 'chords';
  originalKey?: string;
  targetKey?: string;
  pitch: number;
  isPlayed?: boolean;
  is_confirmed_for_set?: boolean;
  bpm?: string;
  genre?: string;
  isSyncing?: boolean;
          ...
          <table className="w-full border-collapse min-w-[900px]">
            <thead>
              <tr className="bg-slate-50/50 dark:bg-slate-900/50 border-b dark:border-slate-800">
                <th className="py-3 px-6 text-[10px] font-black uppercase tracking-[0.2em] text-slate-400 w-16 text-center">Done</th>
                <th className="py-3 px-6 text-[10px] font-black uppercase tracking-[0.2em] text-slate-400 w-16 text-center">Set</th>
              ...
              {processedSongs.map((song, idx) => {
                ...
                return (
                  <tr 
                    ...
                    className={cn(
                      "transition-all group relative cursor-pointer h-[85px]",
                         ...
                         <Tooltip>
                           <TooltipTrigger asChild>
                             <button 
                               onClick={(e) => { 
                                 e.stopPropagation(); 
                                 onUpdateSong(song.id, { is_confirmed_for_set: !song.is_confirmed_for_set }); 
                               }}
                               className={cn(
                                 "transition-all active:scale-90 inline-flex items-center justify-center h-8 w-8 rounded-xl border-2",
                                 song.is_confirmed_for_set 
                                   ? "bg-indigo-600 border-indigo-500 text-white shadow-lg shadow-indigo-600/20" 
                                   : "border-slate-100 dark:border-slate-800 text-slate-300 hover:border-indigo-200"
                               )}
                             >
                               {song.is_confirmed_for_set ? <CheckSquare className="w-4 h-4" /> : <Square className="w-4 h-4" />}
                             </button>
                           </TooltipTrigger>
                           <TooltipContent className="text-[10px] font-black uppercase">Include in Set Time Goal</TooltipContent>
                         </Tooltip>
                       </TooltipProvider>
                    </td>
                    ...
                    <td className="px-6 text-center">
                      <div className="flex items-center justify-center gap-4 h-full">
                        <div className="text-center min-w-[32px]">
                          ...
                          <span className="text-xs font-mono font-bold text-slate-500 block leading-none">{formatKey(song.originalKey, currentPref)}</span>
                        </div>

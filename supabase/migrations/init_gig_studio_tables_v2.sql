...
DROP POLICY IF EXISTS "Users can manage their own gig sessions" ON public.gig_sessions;
CREATE POLICY "Users can manage their own gig sessions" ON public.gig_sessions
    FOR ALL USING (auth.uid() = user_id);

DROP POLICY IF EXISTS "Public can view active gig sessions by code" ON public.gig_sessions;
CREATE POLICY "Public can view active gig sessions by code" ON public.gig_sessions
    FOR SELECT USING (is_active = true);

-- Policies for Sheet Links
DROP POLICY IF EXISTS "Users can manage their own sheet links" ON public.sheet_links;
...
DROP POLICY IF EXISTS "Anyone can see follow counts" ON public.follows;

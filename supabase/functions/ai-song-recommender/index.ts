// AI Song Recommender Edge Function
...
Example: [{"id": "uuid", "reason": "Perfect high-energy transition after the mid-set ballad."}]`;
    ...
    for (const provider of shuffledProviders) {
      try {
        console.log(`[ai-song-recommender] Attempting suggestions via ${provider.type.toUpperCase()} (${provider.name})...`);
        let content = "";
        ...
        console.warn(`[ai-song-recommender] Provider ${provider.name} failed: ${err.message}`);
        lastError = err;
        continue;
      }
    }
    ...
    console.error("[ai-song-recommender] Error:", error.message);
